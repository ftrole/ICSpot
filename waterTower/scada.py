...
def waterLevels():
    ...
    while not thread_stop_event.isSet():
        ...
        waterLevel = float(level[0][1])
        waterLevel*=TANK_SECTION
        ...
        cursorObj.execute('SELECT name, value FROM swat_s1 WHERE name="FIT101"')
        FIT101 = cursorObj.fetchall()[0][1]

        cursorObj.execute('SELECT name, value FROM swat_s1 WHERE name="FIT201"')
        FIT201 = cursorObj.fetchall()[0][1]

        db.commit()
        
        socketio.emit('newnumber', {'number': waterLevel, 'MV001' : MV001, 'P201': P201, 'FIT101': FIT101, 'FIT201': FIT201}, namespace='/test')
        socketio.sleep(0.3)
